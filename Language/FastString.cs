...
public class FastString
{
    ...
    //两种情况为空
    //m_isStringGenerated = false, 且m_bufferPos = 0
    //m_isStringGenerated = true，且m_stringGenerated = null
    public bool IsEmpty()
    {
    ...
    public FastString Append( int value )
    {
        ...
        //注意，一个char 2个字节，16个char，32个字节，可以包含int
        ReallocateIFN( 16 );
    ...
    public FastString Append( float valueF )
    {
        ...
        //single的精度有效位数是7位，因此保证精度尽可能大于100w
        int nbDecimals = 0;
        while( value < 1000000 )
        {
            value *= 10;
    ...
    public FastString Replace( string oldStr, string newStr )
    {
        ...
        //优化：
        //m_bufferPos如果比oldStr.Length小的情况也不需要比较
        //m_bufferPos在最后一个有效字符的后一个位置, 这个是和oldStr比较的最低长度
        if (m_bufferPos < oldStr.Length)
            return this;
